...
import { actionTeamList,actionTeamSetCurrent  } from "../../redux/actions/team";
...
const AdminTeam = props => {
  ...
  const onTeamClick = (key) => {
    dispatch(actionTeamSetCurrent(teams[key]));
    history.push('/admin/teams/update');
  }
            ...
            <div className="w-100 overflow-auto">
              <TableBs striped hover variant="dark" style={{ margin: 0 }} className="text-center">
                ...
                <tbody>
                  {
                    teams.map((item, key) => 
                      <tr key={key} onClick={() => onTeamClick(key)} style={{ cursor: 'pointer' }}>
                        <td>{key + 1}</td>
                        <td>{item.name}</td>
                        <td>{item.job}</td>
                        <td>{item.content}</td>
